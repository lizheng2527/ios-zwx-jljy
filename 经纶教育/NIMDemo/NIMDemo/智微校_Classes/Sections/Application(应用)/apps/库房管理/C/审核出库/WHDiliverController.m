...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if(indexPath.section == 1)
    {
        ...
        cell.itemPriceTextField.text = [_mainModel.itemListArray[indexPath.row] cost];
        
        WHMyItemListModel *listModel =_mainModel.itemListArray[indexPath.row];
        if ([NSString isBlankString:listModel.cost]) {
            listModel.cost = @"0";
        }
        
        if ([NSString isBlankString:listModel.count]) {
            listModel.count = @"0";
        }
        ...
        cell.goodAllPrice.text = [NSString stringWithFormat:@"%ld", (listModel.count.integerValue * listModel.cost.integerValue)];
        return cell;
...
-(void)itemWillAdd:(WHApplicationItemCell *)cell
{
    ...
    NSInteger cost = [model.cost integerValue];
    NSString *allPrice = [NSString stringWithFormat:@"%ld",cost * num];
    cell.goodAllPrice.text = allPrice;
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string;
{
    ...
    else
    {
        for (int i = 0; i < _mainModel.itemListArray.count; i++) {
            ...
            WHApplicationItemCell *cell = [_mainTableView cellForRowAtIndexPath:indexPath];
            WHMyItemListModel * model = _mainModel.itemListArray[indexPath.row];
            ...
            if (tagCount == textField.tag) {
                ...
                cell.goodAllPrice.text = [NSString stringWithFormat:@"%ld",model.count.integerValue * model.cost.integerValue];
                return YES;
            }
            if (tagPrice == textField.tag) {
                NSString * toBeString = [textField.text stringByReplacingCharactersInRange:range withString:string];
                model.cost = [NSString stringWithFormat:@"%@",toBeString];
                cell.goodAllPrice.text = [NSString stringWithFormat:@"%ld",model.count.integerValue * model.cost.integerValue];
                return YES;
            }
        }
